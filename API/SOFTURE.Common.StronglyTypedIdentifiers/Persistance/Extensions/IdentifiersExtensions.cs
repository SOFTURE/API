...
internal static class IdentifiersExtensions
{
    ...
    public static void ConfigureStronglyIdentifiers<TLanguageAssemblyMarker>(this ModelConfigurationBuilder configurationBuilder)
        where TLanguageAssemblyMarker : IAssemblyMarker
    {
        ...
        foreach (var identifierType in identifierTypes)
        {
            ...
            {
                configurationBuilder.RegisterConverter(identifierType, typeof(long));
            }
        }
    }
    ...
    private static void RegisterConverter(this ModelConfigurationBuilder configurationBuilder, Type identifierType, Type valueType)
    {
        ...
        {
            var method = typeof(IdentifiersExtensions)
                .GetMethod(nameof(ConfigureLongIdentifier), BindingFlags.Static | BindingFlags.NonPublic)
                ?.MakeGenericMethod(identifierType);
    ...
    private class LongToIdentifierConverter<TIdentifier>() : ValueConverter<TIdentifier, long>(id => id.Value,
        value => (TIdentifier)Activator.CreateInstance(typeof(TIdentifier), value)!)
        where TIdentifier : IdentifierBase<long>;
    ...
    private static void ConfigureLongIdentifier<TIdentifier>(ModelConfigurationBuilder configurationBuilder)
        where TIdentifier : IdentifierBase<long>
    {
        configurationBuilder
            .Properties<TIdentifier>()
            .HaveConversion<LongToIdentifierConverter<TIdentifier>>();
    }
}